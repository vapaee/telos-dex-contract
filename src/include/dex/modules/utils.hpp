    ...
    namespace dex {
        namespace utils {
            ...
            inline uint128_t combine( uint64_t key1, name key2 ) {
                ...
                return eosio::dex::utils::combine(key1, key2.value);
            }

            inline uint128_t combine( name key1, name key2 ) {
                return eosio::dex::utils::combine(key1.value, key2.value);
            }
            ...
            asset string_to_asset(const std::string& str) {
                // PRINT("eosio::dex::utils::string_to_asset()\n");
                // PRINT(" str: ", str.c_str(), "\n");

                string s = str;
                s = eosio::dex::utils::trim(s);
                ...
                std::string symbol_str = eosio::dex::utils::trim(substring);
                ...
                if (dot_pos != string::npos) {
                    ...
                    int_part = eosio::dex::utils::str_to_int64(amount_str.substr(0, dot_pos));
                    fract_part = eosio::dex::utils::str_to_int64(amount_str.substr(dot_pos + 1));
                    if (amount_str[0] == '-') {
                        fract_part *= -1;
                    }
                    ...
                    int_part = eosio::dex::utils::str_to_int64(amount_str);
                }
            ...
            inline name aux_get_modify_payer(name owner) {
                if (owner == eosio::dex::contract) return same_payer;
